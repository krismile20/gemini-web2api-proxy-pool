...
lines = [
    ...
    "",
    "pool:",
    "  mode: %s" % yq(env.get("POOL_MODE", "random")),
    "  failure_threshold: %s" % int(env.get("POOL_FAILURE_THRESHOLD", "3") or 3),
    "  blacklist_duration: %s" % yq(env.get("POOL_BLACKLIST_DURATION", "1h")),
    "  retry_enabled: true",
    "  retry_attempts: 3",
    "",
    "sticky:",
